...
class PosConfig(models.Model):
    ...
    display_total_variant_stock = fields.Boolean(
        default=False,  string="Display Total Variant Stock on Product Card", help="Enable to show the total available stock (sum of all variants) on the product card in the POS screen. If disabled, stock per variant will be displayed individually.")
    ...
    picking_type_location_id_num = fields.Integer(
        compute='_compute_picking_type_location_id_num', string="Default Source Location id")

    @api.depends('picking_type_location_id')
    def _compute_picking_type_location_id_num(self):
        for config in self:
            config.picking_type_location_id_num = config.picking_type_location_id.id
